...
import { CheckCircle2, XCircle, RotateCcw, Sparkles, MessageSquareText, Eye, EyeOff } from "lucide-react";
...
const ContextHardExercise = () => {
  ...
  const handleCheckAnswer = () => {
    ...
    const isCorrect = userInput.trim().toLowerCase() === currentExercise.word.term.toLowerCase();
  ...
  const renderSentenceWithInput = () => {
    ...
    return <div className="flex flex-wrap items-center justify-center gap-2 text-2xl font-medium text-foreground">
        <span>{parts[0]}</span>
        <Input ref={inputRef} type="text" value={userInput} onChange={e => setUserInput(e.target.value)} onKeyDown={handleKeyDown} disabled={answerState !== "unanswered"} placeholder="Type here..." className={`inline-flex h-auto min-w-[140px] max-w-[200px] px-4 py-2 text-xl font-semibold text-center ${getInputStyle()}`} />
        <span>{parts[1]}</span>
      </div>;
  };
            ...
            duration: 0.3
                ...
                <div className="mb-8 overflow-hidden rounded-2xl border border-border bg-card shadow-lg">
                  ...
                  <div className="border-t border-border bg-muted/30 p-4 text-center">
                    <button onClick={toggleHint} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                      {showHint ? <>
                    ...
                    {showHint && <p className="mt-2 text-sm text-muted-foreground italic">
                        {currentExercise.word.sentenceTranslation.replace(currentExercise.word.definition.toLowerCase(), "___")}
                      </p>}
                  </div>
                </div>
              ...
              y: 0
                    ...
                    <Button onClick={handleCheckAnswer} className="w-full" disabled={!userInput.trim()}>
                      Check Answer
                    </Button>
                  </motion.div>}
